...
export async function sendRenewalRetryFailureEmail(email, firstName, month) {
  try {
    ...
    console.log('Error sending renewal retry failure email for member ' + member.id + ': ', error);
  }
}
...
When we attempted to renew your membership your credit card failed. We've extended the picking period just for you, so there is still time to get your ${monthName} book.
